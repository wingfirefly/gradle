...
public abstract class AbstractTablePageGenerator extends HtmlPageGenerator<ResultsStore> {
    ...
    protected abstract int howManyMostRecentExecutionsRead();
    ...
    protected Set<ScenarioBuildResultData> readBuildResultData(File resultJson) {
        try {
            ...
            // @formatter:on
            return sortBuildResultData(list.stream().map(scenario -> queryExecutionData(scenario)));
        } catch (IOException e) {
    ...
    private ScenarioBuildResultData queryExecutionData(ScenarioBuildResultData scenario) {
        PerformanceTestHistory history = resultsStore.getTestResults(scenario.getScenarioName(), howManyMostRecentExecutionsRead(), PERFORMANCE_DATE_RETRIEVE_DAYS, ResultsStoreHelper.determineChannel());
